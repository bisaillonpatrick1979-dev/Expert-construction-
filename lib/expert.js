/**
 * Expert Construction IA — logique centrale
 *
 * Handlers HTTP « vanilla » (http.ServerResponse standard) : ils fonctionnent
 * identiquement en local (server.js) et comme fonctions serverless Vercel
...
/* ------------------------------------------------------------------ */
/* Utilitaires HTTP « vanilla » (compatibles local + Vercel)           */
/* ------------------------------------------------------------------ */
...
async function readJsonBody(req) {
  if (req.body !== undefined) {
    // Vercel a déjà analysé le corps JSON
...
function handleHealth(_req, res) {
  sendJson(res, 200, { ok: true, model: MODEL, keyConfigured: Boolean(process.env.ANTHROPIC_API_KEY) });
}

module.exports = { handleChat, handleHealth, MODEL };
